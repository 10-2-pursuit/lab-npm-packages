...
function sumNumbers(array) {
  const noFalsey=_.compact(simpleArray);
   const sumTruthy = _.sum(noFalsey);
return sumTruthy;
}
...
function newMemberArrayToObject(member) {
  const newMemberObject=_.fromPairs(newMember)
...
function groupClassByInstructor(yogaClasses) {
  for(const yogaClass of yogaClasses)
  return _.groupBy(yogaClasses,'instructor')
  }
...
function omitAgeFromMembers(members) {
  return members.map(member => _.omit(member, 'age'))
}
...
function countClassesByInstructor(yogaClasses, instructorName) {
  const countByInstructor =_.countBy(yogaClasses, 'instructor');
  const count = countByInstructor[instructorName] || 0;
...
function removeInactiveMembers(collection) {
 return _.filter(members, {currentMember:true})
}
...
function getUniqueClasses(yogaClasses) {
  const uniqueClasses = _.uniqBy(yogaClasses, 'title');
  //cls represents one of the uniqueClasses
  return uniqueClasses.map(cls => ({ title: cls.title, priceInCents: cls.priceInCents }));
}
...
function orderClassesByTitleAndLevel(collection) {
  return _.orderBy(collection, ['title', 'level'], ['asc', 'desc']).map(classObj => {
    return {
