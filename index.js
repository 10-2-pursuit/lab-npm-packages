...
function sumNumbers(array) {
  return _.compact(array).reduce((accumulator, currentValue) => accumulator + currentValue, 0);
}
...
function omitAgeFromMembers(collection) {
  ...
  for (let member of collection) {
    finalArr.push(_.omit(member, "age"));
  }
  return finalArr;
}
...
function countClassesByInstructor(collection, instructor) {
  return !_.find(collection, classes => classes.instructor === instructor) ? `There is no instructor by that name.` : _.filter(collection, classes => classes.instructor === instructor).length;
}
...
function removeInactiveMembers(collection) {
  return _.filter(collection, classes => classes.currentMember === true);
}
...
function getUniqueClasses(collection) {
  return _.map(_.uniqBy(collection, "title"), classes => {return {title: classes.title, priceInCents: classes.priceInCents}});
}
...
function orderClassesByTitleAndLevel(collection) {
  return _.orderBy(_.map(collection, (classes) => {return {title:classes.title, instructor:classes.instructor, level:classes.level}}), ["title", "level"], ["asc", "desc"]);
}
