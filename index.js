...
function omitAgeFromMembers(collection) {
  return _.omitBy(collection, "age");
}
...
function countClassesByInstructor(collection, instructor) {
  ...
  const result = _.filter(collection, ['instructor', instructor]).length;
...
function getUniqueClasses(collection) {
  const result = _.map(_.uniqBy(collection, 'title'), function (item) {
    return {
...
function orderClassesByTitleAndLevel(collection) {
  let newArr = collection.map((classObj) =>
