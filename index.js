...
function sumNumbers(array) {
  const filteredArray = _.filter(array, _.isNumber);
  return _.sum(filteredArray);
}
...
function countClassesByInstructor(collection, instructor) {
  const classes = _.filter(collection, {instructor})
  const classCount = _.size(classes)
  if (classCount === 0) {
...
function getUniqueClasses(collection) {
  return _.chain(collection)
    .groupBy('title')
    .map(group => _.pick(group[0], ['title', 'priceInCents']))
    .value()
}
...
function orderClassesByTitleAndLevel(collection) {
  return _.orderBy(collection, ['title', 'level'], ['asc', 'desc'])
    .map(classObj => _.pick(classObj, ['title', 'instructor', 'level']))
}
