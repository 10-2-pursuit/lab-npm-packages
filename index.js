...
function sumNumbers(array) {
  let addedNums = array.filter(element => typeof element === 'number')
  return _.sum(addedNums);
}
...
function newMemberArrayToObject(member) {
  const memberArrToObj = _.fromPairs(member);
  return memberArrToObj;
}
...
function omitAgeFromMembers(collection) {
  let omittedAge = _.map(collection, member => _.omit(member, 'age'));
  return omittedAge;
}
...
function countClassesByInstructor(collection, instructor) {
  if (!_.filter(collection, {instructor: instructor}).length) {
    return 'There is no instructor by that name.'
  } else {
    return _.filter(collection, {instructor: instructor}).length
  }
}
