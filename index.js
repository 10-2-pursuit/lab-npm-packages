...
const removedAge = require("./data/members-without-ages.json");
...
function omitAgeFromMembers(collection) {
  ...
  for (let member of collection) {
    withoutAge.push(_.omit(member, "age"));
  }
  return withoutAge;
...
function countClassesByInstructor(collection, instructor) {
  let classesByInstructor = _.filter(collection, {"instructor": instructor}).length
  if (classesByInstructor > 0) {
